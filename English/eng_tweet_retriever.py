...
def tweet_filter(text):
	...
	punct_regex = r'(\-|\:|\!|\%|\*|\)|\(|\+|\-|\/|\$|\'|\"|\*|\[|\]|\{|\}|\#|\^|\,|\.)+'
	entity_regex = r'&\w+;'
	horizontal_ellipsis_regex = r'[^\x00-\x7F]+'
	...
	# remove ...
	pattern = re.compile(horizontal_ellipsis_regex)
	text = pattern.sub('',text)	
...
while True:
	_tweet_count = 0
	while _tweet_count < 5000:
		try:
			if max_id <= 0:
				tweets = api.search(q="#GameofThrones", lang="en",count=100)
			else:
				tweets = api.search(q="#GameofThrones", lang="en", max_id=str(max_id - 1),count=100)
