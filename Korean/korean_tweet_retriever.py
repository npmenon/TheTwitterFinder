...
def tweet_filter(text):
	...
	kaomojis_regex = re.compile(u'[^\d]\(?(?!['u'\u0030-\u0039'
	u'\u0041-\u007A'u'\uC885'u'\u007C'u'\u003A])['
	u'\u1F600-\u1F6FF'
	u'\u3160'u'\u315c'u'\u3142'u'\u2605'
	u'\u3147'u'\u3145'u'\u2022'u'\u2200'u'\u25D5'
	u'\uFF61'u'\u203F'u'\u261E'u'\uFE35]+\)?', re.UNICODE)
	...
	# remove few punctuations
	pattern = re.compile(before_punc_regex)
	text = pattern.sub('',text)		

	# remove kaomojis
	emojis = None
	match = kaomojis_regex.findall(text)
	if match:
		emojis = match
	text = kaomojis_regex.sub('',text)	
...
def stream_twitter():
	...
	twitterStream.filter(track=['왕좌의 게임'], language="ko")
	twitterStream.filter(language="ko")
...
# target = open('index4_korean.jsonl','a')
# document,_tweet_count, _max_id = load_document(tweets, topic, _tweet_count)
